defmodule EventStore.Storage.ReadEventsTest do
  ...
  describe "read stream backward" do
    test "when stream does not exist", %{conn: conn} do
      {:ok, []} = Storage.read_stream_backward(conn, 1, 0, 1_000)
    end

    test "when empty", %{conn: conn} do
      {:ok, _stream_uuid, stream_id} = create_stream(conn)

      assert {:ok, []} = Storage.read_stream_backward(conn, stream_id, 1, 1_000)
    end
    ...
    test "with single event", %{conn: conn} do
      ...
      assert {:ok, [read_event]} = Storage.read_stream_backward(conn, stream_id, 1, 1_000)
    ...
    test "with multiple events from end limited by count", %{conn: conn} do
      {:ok, _stream_uuid, stream_id} = create_stream_containing_events(conn, 10)
      # 10, 9, 8, 7, 6
      {:ok, read_events} = Storage.read_stream_backward(conn, stream_id, 10, 5)

      assert length(read_events) == 5
      assert pluck(read_events, :event_number) == Enum.to_list(10..6)
    end

    test "with multiple events from stream version limited by count", %{conn: conn} do
      {:ok, _stream_uuid, stream_id} = create_stream_containing_events(conn, 10)

      {:ok, read_events} = Storage.read_stream_backward(conn, stream_id, 5, 5)

      assert length(read_events) == 5
      assert pluck(read_events, :event_number) == Enum.to_list(5..1)
    end
  end

  describe "read all streams backward" do

    test "when no streams exist", %{conn: conn} do
      {:ok, []} = Storage.read_stream_backward(conn, @all_stream_id, 1, 1_000)
    end
    ...
    test "with multiple events", %{conn: conn} do
      ...
      {:ok, events} = Storage.read_stream_backward(conn, @all_stream_id, 4, 1_000)

      assert length(events) == 4
      assert [4, 3, 2, 1] == Enum.map(events, &(&1.event_number))
      assert [stream2_uuid, stream1_uuid, stream2_uuid, stream1_uuid] == Enum.map(events, &(&1.stream_uuid))
      assert [2, 2, 1, 1] == Enum.map(events, &(&1.stream_version))
    end

    test "with multiple events starting from origin", %{conn: conn} do
      {:ok, stream1_uuid, stream1_id} = create_stream(conn)
      {:ok, stream2_uuid, stream2_id} = create_stream(conn)
      ...
      {:ok, events} = Storage.read_stream_backward(conn, @all_stream_id, 0, 1_000)
    ...
    test "with multiple events from after last event limited by count", %{conn: conn} do
      ...
      {:ok, events} = Storage.read_stream_backward(conn, @all_stream_id, 20, 10)

      assert length(events) == 10
      assert pluck(events, :event_number) == Enum.to_list(20..11)

      {:ok, events} = Storage.read_stream_backward(conn, @all_stream_id, 10, 10)

      assert length(events) == 10
      assert pluck(events, :event_number) == Enum.to_list(10..1)
    end
  end
